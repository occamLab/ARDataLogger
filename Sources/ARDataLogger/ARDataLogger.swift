...
public class ARLogger: ARDataLoggerAdapter {
    ...
    func toLogFrame(frame: ARFrame, type: String, meshLoggingBehavior: MeshLoggingBehavior)->ARFrameDataLog? {
        ...
        return ARFrameDataLog(timestamp: frame.timestamp, type: type, jpegData: jpegData, cameraTransformInverse: frame.camera.transform.inverse, rawFeaturePoints: frame.rawFeaturePoints?.points, projectedFeaturePoints: frame.rawFeaturePoints?.points.map({feature in frame.camera.projectPoint(feature, orientation: .landscapeRight, viewportSize: frame.camera.imageResolution)}), depthData: transformedCloud, intrinsics: frame.camera.intrinsics, planes: frame.anchors.compactMap({$0 as? ARPlaneAnchor}), pose: frame.camera.transform, meshes: meshes)
    }
